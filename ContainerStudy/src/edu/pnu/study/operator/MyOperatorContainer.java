...
//	private ArrayList<MyOperator> container = new ArrayList<>();
	...
	public int doOperate(String cmd, int f, int s) {
		...
		return mo.operate(f,  s);  // 다형성(polymorphism)
	}
	...
	public void releaseOldObject(int millisecond) {
		...
		for (String key : keys) {
			...
			MyOperator mo = container.get(key);  // 해당 key 값 날리기
		...
		// 컨테이너에서 오퍼레이터 객체들 중 second 시간만큼 사용된 적이 없는 객체를 검색한다.
//		for (int i = size - 1 ; 0 < i ; i--) { // 0 <= i인 동안 돌아가면, Exception in thread "Thread-0" java.lang.NullPointerException: Cannot invoke "edu.pnu.study.operator.MyOperator.getLastUsedTime()" because "mo" is null
